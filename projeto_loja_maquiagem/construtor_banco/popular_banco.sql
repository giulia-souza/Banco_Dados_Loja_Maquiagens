...
-- A. Calculando o valor das Vendas
UPDATE Vendas v
JOIN (
    SELECT id_venda, SUM(quantidade * preco_unitario) as total_calculado
    FROM Itens_Venda
    GROUP BY id_venda
) i ON v.id_venda = i.id_venda
SET v.valor_total = i.total_calculado;

-- B. Calculando o valor dos Pedidos de Compra
UPDATE Pedidos_Compra p
JOIN (
    SELECT id_pedido_compra, SUM(quantidade_comprada * preco_custo_unitario) as custo_calculado
    FROM Itens_Pedido_Compra
    GROUP BY id_pedido_compra
) i ON p.id_pedido_compra = i.id_pedido_compra
SET p.valor_total_custo = i.custo_calculado;

-- C. Auditoria de Estoque: (Comprado) - (Vendido)
UPDATE Produtos p
LEFT JOIN (
    SELECT id_produto, SUM(quantidade_comprada) as total_comprado
    FROM Itens_Pedido_Compra
    GROUP BY id_produto
) compras ON p.id_produto = compras.id_produto
LEFT JOIN (
    SELECT id_produto, SUM(quantidade) as total_vendido
    FROM Itens_Venda
    GROUP BY id_produto
) vendas ON p.id_produto = vendas.id_produto
SET p.quantidade_estoque = IFNULL(compras.total_comprado, 0) - IFNULL(vendas.total_vendido, 0);